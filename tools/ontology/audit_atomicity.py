...
def audit(model: dict) -> list[dict]:
    ...
    for t in writes:
        ...
        if _is_branch_dispatch(t["src"]):
            ...
            continue
        out.append(_finding(
            "AT-01", "error", f"复合动词无补偿：{t['tool']} 单次调用顺序横跨 {len(weps)} 个写端点",
            f"src/kingdee_mcp/server.py:{t['line']}",
    ...
    if loops and joins:
        out.append(_finding(
            "AT-02", "error", "批量语义分裂：同为 bill_ids，两种原子性级别",
            "server.py:_run_execute_action / kingdee_submit_bills 等",
            f"逐条循环（部分成功、可报告 per-id 结果）：{loops}；"
            f"Ids 逗号拼接单次提交（原子性由服务端决定、返回体无 per-id 结果）：{joins}。"
            f"调用方无法从工具签名区分二者，失败后不知道哪些 id 已生效。"))
    ...
    uncovered = sorted({e for t in writes for e in t["endpoints"]
                        if e in WRITE_ENDPOINTS and e not in covered})
    if uncovered:
        out.append(_finding(
            "AT-04", "error", f"状态未定义：{len(uncovered)} 个写端点不在 DOC_LIFECYCLE 中",
            f"server.py:DOC_LIFECYCLE（仅 {sorted(covered)}）",
            f"未定义端点 {uncovered}。_result_status() 对其 lifecycle 取空字典，"
            f"next_action 一律返回 None，等价于向调用方宣告『流程已完成』，"
            f"掩盖了作废/关闭/禁用后的真实状态。"))

    # ── AT-05 规则覆盖 ────────────────────────────────────────────
    rules_src = RULES_PY.read_text(encoding="utf-8") if RULES_PY.exists() else ""
    named = set(re.findall(r'"(kingdee_[a-z_]+)"', rules_src))
    missed = sorted(t["tool"] for t in writes if t["tool"] not in named)
    if missed:
        out.append(_finding(
            "AT-05", "error",
            f"约束层覆盖缺口：{len(missed)}/{len(writes)} 个写动词未被任何 harness 规则点名",
            "harness/rules.py（规则以工具名字符串硬编码匹配）",
            f"未覆盖：{missed}。这些工具产生的操作链不受生命周期完整性/下推链完整性约束。"))
    ...
    if model["links"]:
        ...
        out.append(_finding(
            ...
            f"硬编码链接 {hard}，加上 kingdee_push_bill / kingdee_push_and_audit 的自由参数形式，"
