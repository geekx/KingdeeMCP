...
| L-1 | ✅ 已修 | legacy 的全部 6 条下推路径接入同一份链接表。校验点放在 `_post_raw` 的 push 分支——所有路径（含 3 个复合工具）的唯一咽喉点。**默认只警告不阻断**（未登记 ≠ 不合法，链接表只有 9 条而金蝶支持的远不止），`KINGDEE_STRICT_LINKS=1` 时在发请求前拒绝 |
| L-3 | ◐ 部分 | 名词×动词适用性已由底座 PRE-01 校验；legacy 的工具与名词仍无显式绑定 |
| L-2 | ❌ 存疑 | 需在真实账套实证 |
