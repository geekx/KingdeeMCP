...
发现 F-1 的第一版结论是**错的**，值得记下来。

最初抽取「查询工具的默认字段集」时只读了函数体里的 `IfExp` 兜底值，
于是把默认值写在 Pydantic 模型 `field_keys` 上的工具全判成"与注册表不一致"，
报出一份包含 `STK_Inventory`、`BD_Material` 等在内的错误清单。

核对单个案例（`InventoryQueryInput.field_keys` 确实有 default）后才发现是抽取漏洞，
不是真的不一致。补全两处来源重跑，才得到可信的 14 处。

第二个问题紧随其后：同一个 `form_id` 可能有多个查询工具
（`PUR_PurchaseOrder` 有 2 个、`FA_FAGet` 有 2 个），
早期实现用 dict 后写覆盖先写，结果随 AST 遍历顺序漂移、不可复现。
改为「带显式默认值的先到先得」才稳定下来。

教训有两条：

1. **抽取器的覆盖面本身要先验证**，否则"发现"可能只是抽取漏洞；
2. **同一事实有多个来源时，取值规则必须确定**，否则同一份代码两次审计给出不同结论。
